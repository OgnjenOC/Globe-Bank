...
<?php 
//check whether the preview GET parameter is being set or not
$preview = false; // by default set to false
...
<?php 
  ...
  if(isset($_GET['id'])){
      $page_id = $_GET['id'];
      $page = find_page_by_id($page_id, ['visible' => $visible]); //sql query from query_functions.php
      if(!$page){
          redirect_to(url_for('/index.php'));
      }
      $subject_id = $page['subject_id'];
      
      $subject = find_subject_by_id($subject_id, ['visible' => $visible]); // check if subject is visible
      if(!$subject){
          redirect_to(url_for('/index.php'));
      }
